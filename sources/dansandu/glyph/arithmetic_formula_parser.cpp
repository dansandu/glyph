...
static double visitor(const Node& node, std::string_view formula, const std::map<std::string, double>& values) {
    if (node.isToken())
    ...
    if (std::find(std::begin(fallThroughRules), std::end(fallThroughRules), ruleIndex) != std::end(fallThroughRules))
        return visitor(node.getChild(0), formula, values);
    else if (ruleIndex == 1)
        return visitor(node.getChild(0), formula, values) + visitor(node.getChild(2), formula, values);
    else if (ruleIndex == 2)
        return visitor(node.getChild(0), formula, values) - visitor(node.getChild(2), formula, values);
    else if (ruleIndex == 4)
        return visitor(node.getChild(0), formula, values) * visitor(node.getChild(2), formula, values);
        ...
        return std::pow(visitor(node.getChild(0), formula, values), visitor(node.getChild(2), formula, values));
    else if (ruleIndex == 9) {
        ...
        return functions.at(function)(visitor(node.getChild(2), formula, values));
    } else if (ruleIndex == 12)
        return visitor(node.getChild(1), formula, values);
    else
...
double evaluate(std::string_view formula, const std::map<std::string, double>& values) {
    ...
    return visitor(tree, formula, values);
}
